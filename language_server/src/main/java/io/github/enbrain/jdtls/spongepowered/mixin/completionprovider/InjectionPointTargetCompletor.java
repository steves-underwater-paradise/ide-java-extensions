...
public class InjectionPointTargetCompletor implements Completor {
					...
					if (injectionPointAnnotationName.equals(Util.INJECTION_POINT_ANNOTATION)) {
						...
						if (injectionPointType instanceof StringLiteral injectionPointTypeStringLiteral) {
							...
							if (injectorAnnotation != null) {
								Expression targetMethodExpr = getMemberValueInAnnotation(injectorAnnotation, "method");
								if (targetMethodExpr instanceof StringLiteral injectorMethodStringLiteral) {
									...
									switch (injectionPointTypeString) {
										case "INVOKE":
											...
											break;
										case "INVOKE_STRING":
											for (String method : collectStringInvokedMethods(targetClasses, targetMethod)) {
												result.add(new CompletionItem(method, Util.METHOD_ITEM));
											}
											break;
										case "INVOKE_ASSIGN":
											for (String method : collectNonVoidInvokedMethods(targetClasses, targetMethod)) {
												result.add(new CompletionItem(method, Util.METHOD_ITEM));
											}
											break;
										case "FIELD":
											Expression opcodeExpr = getMemberValueInAnnotation(injectionPointAnnotation, "opcode");
											Integer opcode = null;
											if (opcodeExpr != null) {
											...
											for (String field : collectAccessedFields(targetClasses, targetMethod, opcode)) {
												result.add(new CompletionItem(field, Util.FIELD_ITEM));
											}
											break;
										case "NEW":
											for (String field : collectInvokedConstructors(targetClasses, targetMethod)) {
	...
	private static List<String> collectInvokedMethods(List<IType> targetClasses, String method) throws JavaModelException {
	...
	private static List<String> collectStringInvokedMethods(List<IType> targetClasses, String method) throws JavaModelException {
		Set<String> result = new HashSet<>();
		...
		visitMethodInClasses(targetClasses, method, new MethodVisitor(Util.ASM_VERSION) {
			...
			public void visitMethodInsn(int opcodeAndSource, String owner, String name, String descriptor, boolean isInterface) {
				if (this.isLastLdc) {
					...
					if (argTypes.length == 1 && argTypes[0].getClassName().equals("java.lang.String") && returnType.getSort() == Type.VOID) {
						result.add("L" + owner + ";" + name + descriptor);
					}
				}
				this.isLastLdc = false;
	...
	private static List<String> collectNonVoidInvokedMethods(List<IType> targetClasses, String method) throws JavaModelException {
		Set<String> result = new HashSet<>();
	...
	private static List<String> collectInvokedConstructors(List<IType> targetClasses, String method) throws JavaModelException {
		Set<String> result = new HashSet<>();
	...
	private static List<String> collectAccessedFields(List<IType> targetClasses, String method, Integer opcode) throws JavaModelException {
		Set<String> result = new HashSet<>();
	...
	private static void visitMethodInClasses(List<IType> targetClasses, String method, MethodVisitor visitor) throws JavaModelException {
