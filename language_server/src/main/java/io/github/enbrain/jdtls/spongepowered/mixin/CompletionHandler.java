...
public final class CompletionHandler {
	...
	private static final List<Completor> COMPLETORS = List.of(new AccessorFieldCompletor(), new InjectionPointTargetCompletor(), new InjectionPointTypeCompletor(), new InjectorMethodCompletor(),
			new InvokerMethodCompletor(), new ShadowMemberCompletor());
	...
	private static ASTNode parse(ICompilationUnit unit) {
		...
		options.put(JavaCore.COMPILER_COMPLIANCE, Util.getJavaVersion());
		options.put(JavaCore.COMPILER_CODEGEN_TARGET_PLATFORM, Util.getJavaVersion());
		options.put(JavaCore.COMPILER_SOURCE, Util.getJavaVersion());

		parser.setSource(unit);
		parser.setResolveBindings(true);
		parser.setStatementsRecovery(true);

		JavaCore.setComplianceOptions(Util.getJavaVersion(), options);
